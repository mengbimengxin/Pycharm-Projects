...
saver.restore(sess, "D:/Pycharm Projects/learn/model/model.ckpt")
...
path = "C:/Pycharm Projects/learn/photo/3.jpg"
# image_raw = tf.gfile.FastGFile(path, "r").read()
# img = tf.image.convert_image_dtype(image_raw, dtype=tf.float32)
# im = tf.image.resize_images(path, (-1, 28, 28, 1), method=0)

img = Image.open(path).resize((28, 28))
img = (np.array(img)[:, :, 1] / 255.0).reshape((-1, 28 * 28))
img = tf.constant(img, tf.float32)
pre = sess.run(prediction, feed_dict={xs: img, keep_prod: 1.0})
print(pre)
